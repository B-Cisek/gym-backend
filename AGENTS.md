...
Guidelines for AI assistants contributing to Gym Backend.
...
**Architecture:** Layered architecture with separation between Domain, Application, Infrastructure, and Presentation layers. See ARCHITECTURE.md for full details.
...
- Domain entities mapped with Doctrine attributes
...
- Doctrine custom types, adapters, external integrations
...
1. **Domain layer is the source of truth for the model** - entities live in `*/Domain/` and are mapped with Doctrine attributes
...
Domain entities use:
- Doctrine attributes directly in domain classes (`#[Entity]`, `#[Column]`, relations)
- Named constructors (e.g., `User::register()`)
- Value objects for complex types (Email, Id)
- Private constructors and factory methods (`create`, `restore`, etc.)
...
- JWT payload includes `owner_id` when authenticated user has owner role
